import {
  top, left, right, bottom, start
} from '../enums';
import type { Placement, Boundary, RootBoundary } from '../enums';
import type {
  Rect, ModifierArguments, Modifier, Padding
} from '../types';
import getBasePlacement from '../utils/get-base-placement';
import getMainAxisFromPlacement from '../utils/get-main-axis-from-placement';
import getAltAxis from '../utils/get-alt-axis';
import within from '../utils/within';
import getLayoutRect from '../dom-utils/get-layout-rect';
import getOffsetParent from '../dom-utils/get-offset-parent';
import detectOverflow from '../utils/detect-overflow';
import getVariation from '../utils/get-variation';
import getFreshSideObject from '../utils/get-fresh-side-object';


type TetherOffsetProps = {
  popper?: Rect,
  reference?: Rect,
  placement?: Placement
}
...
function preventOverflow({ state, options, name }: ModifierArguments<Options>) {
  ...
  const referenceRect = state?.rects?.reference ?? null;
  ...
  if (checkMainAxis) {
    ...
    const additive = tether ? -(popperRect?.[len] ?? 0) / 2 : 0;

    const minLen = variation === start ? (referenceRect?.[len] ?? 0) : (popperRect?.[len] ?? 0);
    const maxLen = variation === start ? -(popperRect?.[len] ?? 0) : -(referenceRect?.[len] ?? 0);
    ...
    // width or height)
    const arrowLen = within(0, (referenceRect?.[len] ?? 0), arrowRect[len]);

    const minOffset = isBasePlacement
      ? (referenceRect?.[len] ?? 0) / 2
      - additive
      - arrowLen
      - arrowPaddingMin
      - tetherOffsetValue
